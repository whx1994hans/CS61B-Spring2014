...
public class UDGraph
{
  ...
  public UDGraph paths(int length) {
    ...
    if(length == 1){
      for(int i = 0; i < vertices; ++ i){
        for(int j = 0; j < vertices; ++ j){
          if(hasEdge(i, j)) newGraph.addEdge(i, j);
        }
      }
      return newGraph;
    }
    ...
    newGraph = paths(length - 1);
    UDGraph anoGraph = new UDGraph(vertices);
    for(int i = 0; i < vertices; ++ i){
      for(int j = 0; j < vertices; ++ j){
        if(newGraph.hasEdge(i, j)){
          for(int k = 0; k < vertices; ++ k){
            if(hasEdge(j, k)) anoGraph.addEdge(i, k);
          }
        }
      }
    }
  ...
  public static void main(String[] args) {
    ...
    System.out.println("Testing length-1 path");
    System.out.println("The graph of length-1 path is\n" + graph.paths(1));
